...
const store = async (req, res, next) => {
  try {
    ...
    if (orderItems.length === 0) {
      res.json({
        error: 1,
        message: `you are not add anything in cart `,
      });
    } else {
...
const index = async (req, res, next) => {
  try {
    ...
    let count = await Order.find({ user: req.session.userId }).countDocuments();
    let orders = await Order.find({ user: req.session.userId })
      .skip(parseInt(skip))
